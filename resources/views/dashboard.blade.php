    ...
    <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr class="hover:bg-gray-50 transition-colors">
                ...
                <!-- Status -->
                <td class="px-6 py-4 whitespace-nowrap">
                    <select class="text-xs border-gray-300 rounded-md shadow-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
                        <option @if($task->status == 'todo') selected @endif>To Do</option>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium flex justify-end gap-2">
                    ...
                    <form action="{{ route('tasks.destroy', $task) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="px-3 py-1 text-red-600 hover:text-white hover:bg-red-600 rounded-md transition" onclick="return confirm('Delete this task?')">Archive</button>
                    </form>
                </td>
            </tr>
            @empty
